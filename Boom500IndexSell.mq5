...
//| UPDATED: Time-based close now uses SECONDS (CloseAfterSeconds)    |
//+------------------------------------------------------------------+
#property strict
...
input int           PositionsToOpen   = 2;                 // Requested positions to open immediately
...
input int           TimerSeconds      = 2;                 // Timer frequency (seconds)
input int           CloseAfterSeconds = 120;               // Time-based close (seconds); set 0 to disable
...
void OnTimer()
{
   ...
   if(GetMyFirstPosition(g_symbol, ticket, pos_time))
   {
      ...
      if((now - pos_time) >= (long)CloseAfterSeconds)
      {
